...
SOAP_SYSTEM_PROMPT = (
    ...
    "5. Use ICD-9-CM codes (NOT ICD-10). Format: Diagnosis - ICD9: XXX.X\n"
    "6. Differential diagnoses: rank by probability, fold into A: as a single DDx line.\n"
    "7. The plan must only include actions explicitly mentioned or clearly implied in the transcript. "
    "Never add investigations, referrals, or medications not discussed.\n"
...
_BASE_FORMAT = (
    "S:\n"
    "[Chief complaint, onset, duration, severity, quality, associated symptoms, "
    "alleviating/aggravating factors. PMHx, current medications, allergies, FHx, SHx, "
    "ROS - only items mentioned in transcript. Items not discussed -> Not reported.]\n\n"
    "O:\n"
    "[For virtual encounters write: Physical examination: Not performed (virtual encounter). "
    "Then list only patient-reported objective data. "
    "For in-person encounters list vitals and exam findings - ONLY what was explicitly examined.]\n\n"
    "A:\n"
    "[Primary diagnosis] - ICD9: [code]\n"
    "[Secondary diagnoses if applicable] - ICD9: [code]\n"
    "DDx: [Diagnosis 1], [Diagnosis 2], [Diagnosis 3] (ranked by probability)\n\n"
    "P:\n"
    "- Investigations: [only if ordered/discussed, else: None ordered.]\n"
    "- Medications: [drug, dose, route, frequency, duration if stated; else: None prescribed.]\n"
    "- Referrals: [if discussed; else: None.]\n"
